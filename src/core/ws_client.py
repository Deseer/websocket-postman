...
class WebSocketConnection:
    ...
    async def send_and_wait(
        self, message: str | dict, timeout: float = 30.0
    ) -> str | None:
...
class WebSocketClientManager:
    ...
    async def send_to(self, connection_id: str, message: str | dict) -> bool:
        ...
        if conn is None:
            available_ids = list(self._connections.keys())
            logger.warning(
                f"连接不存在: {connection_id}。当前可用连接: {available_ids}"
            )
            return False
        return await conn.send(message)

    async def send_and_wait(
        self, connection_id: str, message: str | dict, timeout: float = 30.0
    ) -> str | None:
        """发送消息并等待响应"""
        conn = self.get_connection(connection_id)
        if conn is None:
